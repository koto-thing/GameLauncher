    ...
    >
      ...
      >
        ...
        <div className="soundtrack-caption">
          ...
          <strong>
            <span className="soundtrack-titles">
              {games.map(/** @brief 全タイトルの最大行数を確保し、表示対象だけを見せる */ (item) => (
                <span key={item.id} className={item.id === game.id ? "" : "soundtrack-title-reserve"} aria-hidden={item.id !== game.id}>
                  {item.title}
                </span>
              ))}
            </span>
            <span className="soundtrack-arrow" aria-hidden="true">↗</span>
          </strong>
        </div>
      </Link>
      {games.length > 1 && (
