...
import { trackLoudness } from "./loudness";
...
export function gameDesign(input: unknown): GameDesign {
  const value = record(input);
  let webgl: GameDesign["webgl"];
  if (value.webgl !== undefined) {
    const shader = record(value.webgl);
    requireValue(typeof shader.fragmentShader === "string" && shader.fragmentShader.trim().length > 0 && shader.fragmentShader.length <= 16000 && !shader.fragmentShader.includes("\u0000"), "GLSLコードは1〜16000文字で入力してください。", "design");
    webgl = { fragmentShader: shader.fragmentShader };
  }
  requireValue(
...
export function trackContent(
  ...
  return {
    ...
    ...(value.loudness === undefined ? {} : { loudness: trackLoudness(value.loudness, assetId(value.audioAssetId)) }),
    imageAssetId: assetId(value.imageAssetId),
    imageAlt: textValue(value.imageAlt, policy.text.imageAltMax, "imageAlt"),
    loop,
    rightsConfirmed: value.rightsConfirmed === true,
