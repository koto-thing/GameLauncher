...
import { normalizationGainDb } from "../../domain/loudness";
...
export class BrowserAudio implements AudioEngine {
  ...
  constructor(
    private readonly runtime: PlayerRuntime,
    private readonly assetUrl: (id: string) => string,
  ) {
    if (
  ...
  private unlock(): AudioContext {
    if (typeof AudioContext === "undefined")
      throw new Error(
        "このブラウザー環境はWeb Audioの音量補正・区間ループに対応していません。対応ブラウザーを利用してください。",
      );
    if (!this.context) {
  ...
  /** @brief 通常音声とPCMループの共通出力へ曲全体の固定ゲインを設定する */
  private applyGain(): void {
    if (this.output && this.context && this.track)
      this.output.gain.setValueAtTime(
        10 **
          (normalizationGainDb(this.track.loudness, this.track.audioAssetId) /
            20),
        this.context.currentTime,
      );
  }
  /** @brief 音量補正とリスナーの音量を合成し、通常再生と区間ループに適用する */
  ...
  async play(): Promise<void> {
    ...
    try {
      if (this.buffer) {
        ...
        if (this.track.loudness || this.context) {
          const context = this.unlock();
          await context.resume();
          if (generation !== this.generation) return;
        }
  ...
  async setRegion(region: LoopRegion | null): Promise<void> {
    ...
    try {
      ...
      const job = (async () => {
        ...
        const response = await fetch(this.assetUrl(track.audioAssetId!), {
          signal,
        });
        if (!response.ok)
