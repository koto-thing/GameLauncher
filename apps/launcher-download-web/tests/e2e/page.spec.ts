...
test("initial HTML, download click, disabled OS and no premature external requests", async ({ page, browserName }) => {
  ...
  await expect(page.getByRole("button", { name: "Linux版（準備中）" })).toBeDisabled();
  ...
  await page.route(config.downloads.windows.url, route => route.fulfill({
  ...
  if (browserName === "webkit") await page.locator("a.download").press("Shift");
  else {
  ...
  expect(await page.locator("a.download").evaluate(el => getComputedStyle(el).outlineStyle)).toBe("solid");
  ...
  const destination = browserName === "webkit"
    ? page.waitForRequest(config.downloads.windows.url)
    : page.waitForEvent("download");
  await page.keyboard.press("Enter");
  expect((await destination).url()).toBe(config.downloads.windows.url);
});
...
test("no JavaScript retains poster, title and usable static download", async ({ browser }) => {
  ...
  await expect(page.locator("a.download")).toHaveAttribute("href", config.downloads.windows.url);
