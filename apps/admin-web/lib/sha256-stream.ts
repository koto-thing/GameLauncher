/**
 * Streaming SHA-256 implementation (FIPS 180-4).
 * Processes chunked Uint8Array data incrementally without buffering entire files in memory.
 */

const K = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);

function rotr(x: number, n: number): number {
  return (x >>> n) | (x << (32 - n));
}

export class Sha256Hasher {
  private h = new Uint32Array([
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
    0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
  ]);
  private buffer = new Uint8Array(64);
  private bufferLength = 0;
  private totalBytes = 0;
  private finalized = false;
  private w = new Uint32Array(64);

  public update(chunk: Uint8Array): this {
    if (this.finalized) {
      throw new Error("Cannot update Sha256Hasher after digest() has been called");
    }
    const length = chunk.length;
    if (length === 0) return this;

    this.totalBytes += length;
    let offset = 0;

    // Fill remaining buffer
    if (this.bufferLength > 0) {
      const needed = 64 - this.bufferLength;
      if (length < needed) {
        this.buffer.set(chunk, this.bufferLength);
        this.bufferLength += length;
        return this;
      }
      this.buffer.set(chunk.subarray(0, needed), this.bufferLength);
      this.processBlock(this.buffer, 0);
      this.bufferLength = 0;
      offset = needed;
    }

    // Process full blocks directly from chunk
    while (offset + 64 <= length) {
      this.processBlock(chunk, offset);
      offset += 64;
    }

    // Keep remainder in buffer
    if (offset < length) {
      const remainder = chunk.subarray(offset);
      this.buffer.set(remainder, 0);
      this.bufferLength = remainder.length;
    }

    return this;
  }

  private processBlock(data: Uint8Array, offset: number): void {
    const w = this.w;
    for (let i = 0; i < 16; i += 1) {
      const pos = offset + i * 4;
      w[i] = ((data[pos] << 24) | (data[pos + 1] << 16) | (data[pos + 2] << 8) | data[pos + 3]) >>> 0;
    }
    for (let i = 16; i < 64; i += 1) {
      const s0 = rotr(w[i - 15], 7) ^ rotr(w[i - 15], 18) ^ (w[i - 15] >>> 3);
      const s1 = rotr(w[i - 2], 17) ^ rotr(w[i - 2], 19) ^ (w[i - 2] >>> 10);
      w[i] = (w[i - 16] + s0 + w[i - 7] + s1) >>> 0;
    }

    let a = this.h[0];
    let b = this.h[1];
    let c = this.h[2];
    let d = this.h[3];
    let e = this.h[4];
    let f = this.h[5];
    let g = this.h[6];
    let h = this.h[7];

    for (let i = 0; i < 64; i += 1) {
      const s1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25);
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + s1 + ch + K[i] + w[i]) >>> 0;
      const s0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22);
      const maj = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = (s0 + maj) >>> 0;

      h = g;
      g = f;
      f = e;
      e = (d + temp1) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (temp1 + temp2) >>> 0;
    }

    this.h[0] = (this.h[0] + a) >>> 0;
    this.h[1] = (this.h[1] + b) >>> 0;
    this.h[2] = (this.h[2] + c) >>> 0;
    this.h[3] = (this.h[3] + d) >>> 0;
    this.h[4] = (this.h[4] + e) >>> 0;
    this.h[5] = (this.h[5] + f) >>> 0;
    this.h[6] = (this.h[6] + g) >>> 0;
    this.h[7] = (this.h[7] + h) >>> 0;
  }

  public digest(): string {
    if (this.finalized) {
      throw new Error("digest() has already been called");
    }
    this.finalized = true;

    const totalBits = this.totalBytes * 8;
    // Append 0x80
    this.buffer[this.bufferLength] = 0x80;
    this.bufferLength += 1;

    if (this.bufferLength > 56) {
      this.buffer.fill(0, this.bufferLength);
      this.processBlock(this.buffer, 0);
      this.bufferLength = 0;
    }

    this.buffer.fill(0, this.bufferLength, 56);
    // Big-endian 64-bit length
    const highBits = Math.floor(totalBits / 0x100000000);
    const lowBits = (totalBits & 0xffffffff) >>> 0;

    this.buffer[56] = (highBits >>> 24) & 0xff;
    this.buffer[57] = (highBits >>> 16) & 0xff;
    this.buffer[58] = (highBits >>> 8) & 0xff;
    this.buffer[59] = highBits & 0xff;
    this.buffer[60] = (lowBits >>> 24) & 0xff;
    this.buffer[61] = (lowBits >>> 16) & 0xff;
    this.buffer[62] = (lowBits >>> 8) & 0xff;
    this.buffer[63] = lowBits & 0xff;

    this.processBlock(this.buffer, 0);

    let hex = "";
    for (let i = 0; i < 8; i += 1) {
      hex += this.h[i].toString(16).padStart(8, "0");
    }
    return hex;
  }
}

/**
 * Computes SHA-256 of a File or Blob in incremental chunks.
 * Memory footprint is strictly bounded to the chunkSize (default 2 MiB).
 */
export async function hashFileInChunks(
  file: Blob,
  onProgress?: (processedBytes: number, totalBytes: number) => void,
  signal?: AbortSignal,
  chunkSize: number = 2 * 1024 * 1024,
): Promise<string> {
  const hasher = new Sha256Hasher();
  const total = file.size;
  let offset = 0;

  while (offset < total) {
    if (signal?.aborted) {
      throw new DOMException("The operation was aborted", "AbortError");
    }
    const end = Math.min(offset + chunkSize, total);
    const slice = file.slice(offset, end);
    const buffer = await slice.arrayBuffer();
    hasher.update(new Uint8Array(buffer));
    offset = end;
    if (onProgress) {
      onProgress(offset, total);
    }
  }

  // If file was 0 bytes, update onProgress
  if (total === 0 && onProgress) {
    onProgress(0, 0);
  }

  return hasher.digest();
}
