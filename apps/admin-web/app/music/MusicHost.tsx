...
export function MusicHost() {
  ...
  useEffect(/** @brief 独立entryを読み込み、画面を閉じると音声・購読も破棄する。 */ () => {
    ...
    script.onerror = /** @brief build未実行を無限読込にせず示す。 */ () => {
      if (active) setError("Music管理を読み込めません。管理用buildを確認してください。");
    };
    document.body.appendChild(script);

    return /** @brief 遅い読み込みと多重mountを無効化する。 */ () => {
      active = false;
      dispose?.();
      script.remove();
    };
