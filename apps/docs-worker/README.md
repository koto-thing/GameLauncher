...
この実装の上限は原稿16KiB、1変更3ファイル、HTTP本文210KB、GitHub応答4MB、1ユーザー未完了10変更、変更10回/分、認証済み操作60回/分、OAuth開始10回/分/IPです。Markdownは600行・開き角括弧512個までです。既存の最大編集原稿は10,260 bytesでした。目次は120項目・3階層以内、frontmatterは8,192文字以内です。GitHub処理は30秒timeout、完全なtree/PR差分を取得できなければ停止します。
