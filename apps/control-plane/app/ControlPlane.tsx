    ...
    <article className={`request-card request-${request.environment}`}>
      ...
      <div className="request-actions">
        ...
        {isOwner && request.state === "ready" && (
          <div className="inline-action vertical">{dashboard.actor.isAdmin && <input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Admin bypassの理由" />}<button className="primary-button" disabled={busy} onClick={() => runAction({ action: "submit_request", requestId: request.requestId, reason }, dashboard.actor.isAdmin ? "Admin bypassを記録しました" : "指名承認を申請しました")}>提出する</button></div>
        )}
