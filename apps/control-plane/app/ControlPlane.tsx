    ...
    <article className={`request-card request-${request.environment}`}>
      ...
      <div className="request-actions">
        ...
        {isOwner && request.state === "ready" && (
          <div className="inline-action vertical">{dashboard.actor.isAdmin && request.environment === "staging" && <input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Admin bypassの理由" />}<button className="primary-button" disabled={busy} onClick={() => runAction({ action: "submit_request", requestId: request.requestId, reason }, dashboard.actor.isAdmin && request.environment === "staging" ? "Admin bypassを記録しました" : "指名承認を申請しました")}>提出する</button></div>
        )}
        ...
        {dashboard.permissions.canAdminister && request.state === "recovery_required" && <div className="inline-action vertical"><input value={safetyReason} onChange={(event) => setSafetyReason(event.target.value)} placeholder="R2の確認内容（10文字以上）" /><button className="danger-outline-button" disabled={busy} onClick={() => runAction({ action: "authorize_recovery", requestId: request.requestId, reason: safetyReason }, "復旧確認を記録し、再試行可能にしました")}>復旧後の再試行を許可</button></div>}
