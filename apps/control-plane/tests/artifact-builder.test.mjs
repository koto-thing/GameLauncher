...
test("createMetadata generates valid release.json matching contracts schema", () => {
  ...
  // Validate against contracts/schemas/game-release-source.schema.json
  const schemaPath = path.resolve(import.meta.dirname, "../../../contracts/schemas/game-release-source.schema.json");
  assert.ok(fs.existsSync(schemaPath), "contracts/schemas/game-release-source.schema.json must exist");
...
test("buildArtifact produces valid artifact result and descriptor", async () => {
  ...
  // Validate descriptor against contracts/schemas/deployment-artifact-descriptor.schema.json
