...
def chunk_file(source: Path, output: Path, base_url: str) -> tuple[list[dict[str, Any]], int, str]:
...
def build_game_release(metadata_path: Path, build_directory: Path, output: Path,
                       base_url: str, private_key: Path, platform: str, architecture: str) -> list[Path]:
    ...
    working_directory = str(validate_relative_path(metadata["workingDirectory"]))
    ...
    for source in sorted(path for path in build_directory.rglob("*") if path.is_file()):
        relative = source.relative_to(build_directory).as_posix()
        validate_relative_path(relative)
        chunks, size, digest = chunk_file(source, output, base_url)
        files.append({
            "path": relative,
            "size": size,
            "sha256": digest,
...
def upload_tree(output: Path, endpoint: str, bucket: str) -> None:
    ...
    for path in immutable + pointers:
        relative = path.relative_to(output).as_posix()
        cache = POINTER_CACHE if path in pointers else IMMUTABLE_CACHE
...
def parse_arguments() -> argparse.Namespace:
    ...
    launcher.add_argument("--locale", choices=["ja-JP", "en-US"], required=True)
    ...
    changelog.add_argument("--locale", choices=["ja-JP", "en-US"], required=True)
