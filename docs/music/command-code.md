...
公開曲詳細で「共有」を押し、共有方法の一つとして「コマンドコード」を開く。同じ曲の同じ予約IDから常に同じ記号図形を生成する。配布画像には曲名と技コードを含む。QRコードは表示・保存・印刷しない。PNG（1200/1600/2400px）、SVG、記号列、通常曲URLを保存・コピーできる。曲名はXMLエスケープして埋め込む。未発行の曲は共有メニュー内に「準備中」と表示する。

「コードの背景」で背景色・画像・WebGLを選ぶ。背景色はカラーピッカー、画像は端末からPNG/JPEG/WebP（10MiB・20MP以下）を選択する。画像は端末内でPNGへ変換しSVGにも埋め込むため、外部ファイルに依存しない。WebGLは公開作品のシェーダーを使い、未設定なら標準シェーダーを使う。保存・印刷には操作時点の静止画を埋め込む。背景選択はその共有画面内の設定で、作品の公開デザインは変更しない。

技コードはPandD Musicの専用読み取りで曲を開く。標準カメラ用QRは非表示とし、デザイン案の検討後に扱いを決める。背景は技コードの枠内に反映し、記号とマーカーの直下および外周の読み取り余白は白く保つ。

管理画面では担当曲の編集ページに「コマンドコード」欄を表示する。既存予約は自動で確認し、未発行なら「コマンドコードを発行」で予約する。閲覧だけでは発行せず、発行だけでは曲を公開しない。公開画面と同じ背景選択・PNG/SVG保存を提供する。「曲名付きで印刷」は背景・曲名・記号図形を含む配布画像と公開URLを印刷する。ブラウザーの印刷ダイアログからPDF保存も選択できる。曲名は公開中なら公開版、未公開なら保存済み下書きを使う。
