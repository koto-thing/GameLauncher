...
コマンドコードv1は `0005_music_command_codes.sql` に永久予約を追加する。コード情報は署名本文と現在snapshotへ含め、公開照会・画像認識は管理サービスから独立させる。コードだけの既存曲反映は現在公開版へ追記し、下書きを公開しない。形式と適用順序は [command-code.md](command-code.md) を参照。
